    ...
    if request.method == "POST":
        ...
        try:
            ...
            for _, row in df.iterrows():
                conn.execute(
                    "INSERT INTO orders (client, date, status, manager) VALUES (?, ?, ?, ?)",
                    (
                        row.get("client"),
                        str(row.get("date")),
                        row.get("status"),
...
def order_chat(order_id):
    ...
    if request.method == "POST":
        ...
        conn.execute(
            "INSERT INTO chat (order_id, message, timestamp) VALUES (?, ?, ?)",
            (order_id, message, datetime.now().isoformat()),
        )
        conn.commit()
    c = conn.cursor()
    c.execute(
        "SELECT message, timestamp FROM chat WHERE order_id = ? ORDER BY id",
        (order_id,),
    )
    messages = c.fetchall()
    conn.close()
